...
- 统一 Prompt Library 面板的配色与控件状态映射，让列表卡片、输入框、搜索框和操作按钮跟随主导航主题变量，减少站点间视觉割裂。
- 调整 Prompt 条目卡片的圆角、阴影、字体尺寸和间距，并优化空状态与滚动条样式，使列表信息密度更稳定、可读性更高。
- 弱化关闭按钮的危险态表达，收敛 Prompt 面板顶部操作区的视觉噪音，让搜索、关闭与列表操作的层级更加一致。
- 同步更新版本信息与发布文档，发布 `v3.1.3`。
