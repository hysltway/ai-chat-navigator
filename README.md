...
- 将扩展构建链路从 CRXJS 配置切换为仓库内的自定义 Node 脚本，改由 `scripts/build.mjs` 生成 `dist/` 产物并直接写入最终 `manifest.json`。
- 把 Manifest 与 Popup 静态资源收敛到 `public/` 目录，并新增 `background.js` 入口与开发期自动重载能力，减少调试时手动刷新扩展和页面的频率。
- 调整内容脚本与 Popup 入口，在开发模式下接入统一的 `dev-reload` 逻辑；同步更新本地开发说明，使 `npm run dev` 的行为与当前实现一致。
- 同步更新版本信息与发布文档，发布 `v3.1.0`。
