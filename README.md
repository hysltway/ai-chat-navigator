...
- 将 Popup 与内容脚本的通用视觉层抽取为共享 UI kit，统一主题变量、基础控件、空状态和样式令牌，减少两套界面的重复实现。
- 重构主导航与 Prompt Library 的渲染和样式接线，改为复用共享的主题替换与 DOM 构建能力，让不同站点下的面板外观和交互反馈更一致。
- 调整 Popup 页面结构与样式依赖，改为注入共享 UI kit 样式并跟随系统明暗主题同步配色，收敛独立维护的弹窗样式。
- 同步更新版本信息与发布文档，发布 `v3.2.0`。
