...
- 将 Popup 设置面板收敛为核心功能开关，移除 Formula Copy 的格式与引擎下拉配置，只保留总开关，降低日常配置复杂度。
- 新增 Prompt Library 入口按钮显示开关，并接入内容脚本与存储变更同步逻辑，可按需隐藏站点内的 Prompt 入口按钮，同时在设置变化后即时生效。
- 抽取共享的 Prompt Entry 设置读写模块，并补齐 Popup 布局与文案细节，让功能开关面板在当前结构下更简洁一致。
- 同步更新版本信息与发布文档，发布 `v3.3.1`。
