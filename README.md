...
- 重做 Popup 的头部与分区视觉层，加入品牌卡片、渐变背景、浮层光感和更明确的区块层级，让扩展入口在小尺寸弹窗中也保持更强识别度。
- 将功能开关重排为说明文案与自定义开关并列的卡片样式，并补齐支持区与增长区的标题、描述和按钮反馈，让核心操作更聚焦、更易扫读。
- 调整 Popup 初始化流程，改为并行读取 Formula Copy 与 Prompt Entry 两组设置，减少弹窗打开后的等待感并让开关状态更快完成同步。
- 同步更新版本信息与发布文档，发布 `v3.3.2`。
