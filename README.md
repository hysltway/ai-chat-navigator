...
- 新增增长统计链路，统一记录导航跳转、Prompt Library 打开、会话收藏和 Popup 打开等关键使用行为，并按自然日累计插件使用天数。
- 新增基于阈值的内容区增长弹窗，在达到一定使用次数与使用天数后，引导用户前往 Chrome Web Store 留评或到 GitHub 点 Star，并支持稍后关闭或永久不再显示。
- 将增长弹窗接入共享链接与多语言文案体系，同时补强展示时机与视觉舞台效果，避免页面尚未稳定时过早弹出，并让提醒层更完整、更有辨识度。
- 同步升级版本信息与发布文档，发布 `v3.5.0`。
