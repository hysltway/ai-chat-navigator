...
- 为导航面板、折叠态按钮、条目卡片和预览层补充更完整的进入/退出动画与悬停反馈，同时兼容 `prefers-reduced-motion`，提升长对话导航时的整体质感。
- 调整折叠模式下的预览隐藏逻辑与导航跳转行为，折叠面板时会立即清空预览状态，并在减少动态偏好下禁用平滑滚动，避免多余动画干扰。
- 重构开发期自动重载流程：内容脚本改为请求后台统一触发扩展重载，并在后台重载后主动刷新受支持站点标签页，减少开发模式下的失步状态。
- 优化 Prompt Library 存储读取时序，在创建、删除与复制等写操作前补充重试读取，降低首次空读导致的数据覆盖风险。
- 同步更新版本信息与发布文档，发布 `v3.1.2`。
