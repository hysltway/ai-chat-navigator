...
- 新增 AI 公式点击复制能力，支持 ChatGPT / Gemini / Claude 场景。
- 支持 `MathML / LaTeX` 复制格式切换，并支持 `Shift + 点击` 强制复制 LaTeX。
- 修复 Gemini 公式区域双层可点击框问题，仅保留最内层可交互节点。
- 公式转换依赖改为插件内置 vendor 文件，避免引用 `examplecode` 导致加载失败。
