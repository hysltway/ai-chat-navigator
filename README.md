...
- 重构 Prompt Library UI 文件结构，拆分样式、渲染与装配入口，降低单文件长度。
- 调整新增 Prompt 流程：默认聚焦表单录入，不显示现有 Prompt 条目；只有主动搜索时才展示匹配结果。
- 优化 Prompt 面板交互与视觉细节，包括警告按需显示、关闭按钮危险色、表单场景可滚动与更低干扰的滚动条样式。
- 移除面板内操作弹窗与删除二次确认，同步更新版本信息与发布文档，发布 `v2.3.3`。
