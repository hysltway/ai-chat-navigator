...
- 将 Gemini 引用回复从直接写入输入框改为发送前的引用卡片预览流，选中文本后会先挂载到编辑区上方，便于在正式发送前确认引用内容。
- 新增对发送按钮点击与回车提交的拦截重放逻辑，发送时会把引用块与现有草稿合并写入编辑器，发送失败时回退原始草稿，减少误发和内容丢失。
- 调整引用卡片在单行预览场景下的垂直对齐与自适应刷新逻辑，让图标、文案和移除按钮在窗口尺寸变化后仍保持稳定布局。
- 同步更新版本信息与发布文档，发布 `v3.3.0`。
