...
- 为 ChatGPT、Gemini、Claude 侧边栏对话条目新增收藏按钮，支持悬停显现、收藏高亮和再次点击取消收藏，减少在长会话列表中反复查找重要对话的成本。
- 将收藏状态持久化到本地存储，并在存储变更后回写侧边栏界面，让刷新页面或多标签页场景下的收藏状态保持一致。
- 接入内容脚本启动流程，并为不同站点适配侧边栏节点定位、路由切换轮询与 DOM 变更同步，确保单页应用切换后仍能稳定挂载收藏入口。
- 同步更新版本信息与发布文档，发布 `v3.2.1`。
