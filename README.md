...
- 为 Gemini 新增选中文本后的引用回复按钮，可将当前选区快速转换为引用块并插入输入框，减少手动复制粘贴与补齐引用格式的操作。
- 将引用回复能力接入内容脚本启动流程，并适配 Gemini 对话区、输入区、滚动定位与选择态刷新逻辑，让按钮只在有效对话选区中出现。
- 修正 Gemini Prompt Library 条目按钮的悬停背景表现，并同步更新扩展 manifest 与商店简介文案，使公开描述覆盖导航、收藏与提示词能力。
- 同步更新版本信息与发布文档，发布 `v3.2.2`。
