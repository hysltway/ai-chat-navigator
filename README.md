...
- 调整 Prompt Library 列表项布局，改为内容区与操作区分层排布，避免长标题或长预览挤压右上角操作按钮。
- 为标题和预览补充更稳定的换行与截断规则，长文本场景下保持两行标题、三行预览的可读性。
- 固定操作按钮在卡片右上角，并为正文预留安全间距，减少按钮遮挡文本的问题。
- 同步更新版本信息与发布文档，发布 `v2.3.5`。
