...
- 新增 Prompt Library：可保存常用提示词，支持搜索、复制、删除，并可一键注入当前站点输入框。
- Prompt Library 适配 ChatGPT、Gemini、Claude 的输入区挂载与主题风格，面板位置会随站点布局自动调整。
- 强化会话索引与 UI 挂载时机：SPA 路由切换、DOM 重建或宿主节点刷新后，导航面板与提示词面板可更稳定地恢复。
- 补全文档与版本信息，同步发布 `v2.3.0`。
