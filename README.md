...
### v0.5.0（2026-02-24）
- 新增导航面板自动深浅主题：无需手动切换，自动跟随站点主题
- ChatGPT 主题判断规则：优先使用 `html.dark` / `html.light`
- Gemini 主题判断规则：优先使用 `body` 的 `color-scheme`
- 删除通用主题兜底判断，减少代码冗余并降低运行开销
- 补充深色配色变量与主题监听逻辑，支持主题切换实时生效
