...
- 将扩展工程迁移到 Vite + TypeScript，新增 `package.json`、`vite.config.ts` 与 `manifest.config.ts`，统一由构建流程生成 MV3 输出。
- 重组源码目录为 `src/` 与 `public/`，把内容脚本、Popup、共享公式设置逻辑和静态资源拆分到更清晰的工程结构中。
- 更新本地开发流程：支持 `npm run dev`、`npm run build`、`npm run typecheck`，并改为从 `dist/` 目录加载扩展。
- 同步更新版本信息与发布文档，发布 `v3.0.0`。
...
- 日常开发：保持 `npm run dev` 运行，修改 Popup 或内容脚本后让 CRXJS 处理增量更新；如果当前站点标签页没有自动应用最新内容脚本，再手动刷新该页面一次
