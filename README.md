...
- 为 Prompt Library 面板补充更完整的进入动效，头部、工具栏、表单区与列表区按层次渐入，打开时的界面反馈更连贯。
- 强化表单辅助区、警告提示和空状态的过渡动画，并统一卡片、按钮与辅助操作的缓动曲线，减少界面切换时的生硬感。
- 为列表卡片和右上角操作按钮增加悬停位移与显隐反馈，同时保留 `prefers-reduced-motion` 降级处理，兼顾动态表现与可访问性。
- 同步更新版本信息与发布文档，发布 `v2.3.6`。
