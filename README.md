...
- 为扩展补齐 Chrome i18n 基础设施，新增 `default_locale`、`_locales` 语言包和共享文案读取工具，让 Manifest、Popup 与内容脚本都能统一读取本地化字符串。
- 将导航面板、Prompt Library、公式复制提示、Gemini 引用回复、侧边栏收藏等用户可见文案与 aria 文本全部接入本地化，并补齐中文、英文、德文、西班牙文、日文语言资源。
- 优化 Popup 的多语言展示细节，放宽标题、描述、版本号和操作按钮的换行与布局约束，避免长文案在窄弹窗里被截断或挤压。
- 同步更新版本信息与发布文档，发布 `v3.4.0`。
