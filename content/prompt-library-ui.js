(() => {
  ...
  function renderPrompts(ui, prompts, armedDeletePromptId, options = {}) {
    ...
    if (!prompts.length) {
      ui.list.appendChild(createEmptyState(Boolean(options.hasQuery)));
      return;
    }

    prompts.forEach((prompt) => {
      ui.list.appendChild(createPromptItem(prompt, armedDeletePromptId));
    });
  }

  function createEmptyState(hasQuery) {
  ...
  function createPromptItem(prompt, armedDeletePromptId) {
    const item = document.createElement('article');
    item.className = 'prompt-item';
    item.dataset.promptId = prompt.id;
    item.dataset.action = 'inject-prompt';
