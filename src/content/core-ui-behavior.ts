  ...
  let state: CoreState;
  let PREVIEW_HIDE_DELAY = 0;
  let MINIMAL_MODE_KEY = '';
  let DEFAULT_NORMAL_PANEL_WIDTH = 0;
  let PANEL_CONTENT_MIN_GAP = 0;
  let ADAPTIVE_INTERSECT_ENTER_GAP = 0;
  let ADAPTIVE_INTERSECT_EXIT_GAP = 0;
  let MESSAGE_SCAN_LIMIT = 0;
  let FULL_WIDTH_IGNORE_RATIO = 0;
  let MESSAGE_CONTENT_SELECTORS = '';
  let MESSAGE_ROLE_SELECTORS = '';
  let MESSAGE_INNER_SELECTORS = '';
  let SCROLL_HIGHLIGHT_WAIT_MS = 0;
  let SCROLL_HIGHLIGHT_DURATION_MS = 0;
  let SCROLL_SETTLE_IDLE_MS = 0;
  let HIGHLIGHT_DARKEN_DELTA = 0;
  ...
  let scrollToMessage: (node: Element) => void = () => {};
  let cachedConversationScrollContainer: HTMLElement | null = null;
  let cachedConversationScrollRoot: Element | null = null;

  function initBehaviorApi(ctx: CoreBehaviorContext) {
    ({
      state,
      PREVIEW_HIDE_DELAY,
      MINIMAL_MODE_KEY,
  ...
  function renderMessages() {
    ...
    uiApi.hidePreview(state.ui!);
    uiApi.setVisible(state.ui!, hasMessages);
    uiApi.renderList(state.ui!, state.messages, { minimalMode: state.effectiveMinimalMode });
  ...
  function shouldEnableAdaptiveMinimal() {
    ...
    if (messageRight === null || !Number.isFinite(messageRight)) {
      return false;
    }
  ...
  function syncAdaptiveMode(force = false) {
    if (state.manualModeOverride) {
      ...
      if (uiApi.setAdaptiveMinimal) {
        uiApi.setAdaptiveMinimal(state.ui!, false);
      }
    ...
    if (uiApi.setAdaptiveMinimal) {
      uiApi.setAdaptiveMinimal(state.ui!, state.adaptiveMinimalMode);
    }
    syncDisplayMode(force || changed);
  }
  ...
  function syncDisplayMode(forceRender = false) {
    ...
    uiApi.setMinimalMode(state.ui!, nextMode);
    renderMessages();
    scheduleMinimalScrollHintUpdate();
  }
  ...
  function refreshPreviewPosition() {
    ...
    const item = ui.body.querySelector(`.nav-item[data-index="${state.previewIndex}"]`);
    if (!item) {
      return;
    }
  ...
  function loadMinimalMode() {
    ...
    return storageApi
      ...
      .then((storedValue) => {
        ...
        void storageApi.setBoolean(MINIMAL_MODE_KEY, legacyValue);
        return legacyValue;
      })
      .catch(() => legacyValue);
  }
  ...
  function saveMinimalMode(value) {
    const storageApi = ns.storage;
    if (storageApi && typeof storageApi.setBoolean === 'function') {
      void storageApi.setBoolean(MINIMAL_MODE_KEY, value);
    }
    try {
  ...
  function handleItemPointerOver(event) {
    ...
    if (!(target instanceof Element) || !state.ui) {
      return;
    }
  ...
  function handleItemPointerOut(event) {
    ...
    if (related instanceof Node && (item.contains(related) || state.ui.preview.contains(related))) {
      return;
    }
    schedulePreviewHide();
  }
  ...
  function handlePreviewPointerLeave(event) {
    ...
    const related = event.relatedTarget;
    if (related instanceof Element && related.closest('.nav-item')) {
      return;
    }
    schedulePreviewHide();
  }
  ...
  function showPreviewForItem(item) {
    ...
    uiApi.showPreview(state.ui!, message, item, { contentRight });
  }
  ...
  function setActiveIndex(nextIndex, force = false) {
    ...
    uiApi.setActiveIndex(state.ui!, nextIndex);
  ...
  function getActiveIndex() {
    ...
    let nearestVisibleIndex: number | null = null;
  ...
  function findScrollableAncestor(startNode): HTMLElement | null {
